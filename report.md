...
* **content** (String, Required): This holds the text contents of the post. It is also mandatoryto prevent empty posts being sent to the database.
* **likes** (Number, Default: `0`): This field serves as a cached counter for the total number of likes. While the count could be derived by calculating the length of the likedBy array, using a separate number field allows for faster read operations when rendering the main feed because the database does not need to compute array lengths for every post on display.
* **createdAt** (Date, Default: Date.now): This field automatically records the time when the post was created. Date is set with Date.now.
...
<p align="center">
  <img src="/imgs/postsdb.png" width="40%" height="40%">
 </p>
...
<p align="center">
  <img src="/imgs/userdb.png" width="40%" height="40%">
 </p>
...
This file defines the Login View. It serves as the frontend interface for user authentication, rendered by the server using the EJS templating engine.
...
* **Login Form** (HTML Form): This form collects user credentials. It is configured with method="POST" and action="/login", ensuring that when the "Log In" button is clicked, the data is securely sent to the server's login route for verification.
* **Input Fields** (HTML Inputs): These are the required text fields for the Username and Password. The name attributes (username, password) must match the variable names expected by the backend controller to process the request correctly.
...
* **Library Imports** (HTML Head): This section loads external resources essential for the page's appearance and behavior. It includes the custom stylesheet (/style.css), the Font Awesome library for scalable vector icons (used in the like button), and the client-side logic script (/script.js) loaded with the defer attribute to ensure the HTML parses fully before execution.
* **Navigation Bar** (HTML Div): This container provides the primary user controls. It includes links to create a new post and to log out, ensuring the user can easily manage their session and content creation.
* **Feed Loop** (EJS Loop): This block iterates through the posts array passed from the server. It dynamically generates HTML for each post, populating fields such as the author's name, the creation timestamp, and the content body. It handles the "empty state" case by displaying a friendly message if the database returns no posts.
* **Interactive Post Actions** (Conditional Logic): This section manages user interactions within each post card.
    * **Content Collapsing**: It calculates whether a post is too long (based on character count or newlines) and conditionally renders a "Show more" button to toggle visibility.
    * **Like Button**: It renders a dynamic button that checks if the current user's ID is in the likedBy array, setting the initial heart icon style (solid or outline) accordingly.
    * **Delete Button**: It uses a conditional check to render a delete form only if the current user is the post's author or holds an 'admin' role, enforcing authorization at the view level.
...
The file serves as the primary touchpoint for user engagement, aggregating data from the backend into an interactive, readable format.

##### 3. newpost.ejs

This file defines the New Post View. It serves as the user interface for authenticated users to draft and submit new content to the application.
...
* **Content Input** (HTML Textarea, Required): This multi-line text input allows users to type their message. It is assigned the name="content" attribute, which corresponds to the variable extracted by the backend logic (req.body.content). The required attribute enforces client-side validation, ensuring empty posts cannot be submitted.
* **Form Actions** (HTML Div): This container organizes the interface controls. It includes the "Post" submission button and a navigation link (← Back to Feed), providing a clear exit path for users who wish to cancel the action and return to the main feed.
...
The file serves as the entry point for data generation, providing a simple, focused environment for user expression.

##### 4. signup.ejs

This file defines the Signup View. It serves as the frontend interface for new user registration, allowing visitors to create an account and access the application.

The file contains three key structural components required for the registration functionality.

* **Error Handling Block** (EJS Condition): This section checks if an error variable was passed from the server (e.g., "Passwords do not match"). If present, it dynamically renders a warning message to guide the user in correcting their input.
...
* **Credential Inputs** (HTML Inputs, Required): These fields capture the necessary user data: a unique Username, a Password, and a Confirm Password field. All are marked as required to prevent incomplete submissions, and the confirmation field allows the backend to verify that the user did not make a typo in their password.
...
This file defines the Application Entry Point (app.js). It serves as the central server file, orchestrating database connections, middleware configuration, and routing logic for the application.
...
* **Routing** (URL Mapping): This final section maps HTTP endpoints (GET and POST) to their respective controller functions. It defines routes for rendering pages (Login, Signup, New Post) and handling actions (API calls), before finally starting the server on port 3000.
...
* **Content Toggle** (Event Delegation): This section manages the "Show more/Show less" functionality for long posts. Instead of attaching listeners to every button individually, it uses a single listener on the document. When a click is detected on a .btn-toggle-view element, the script identifies the associated content container and toggles the .collapsed CSS class, dynamically updating the button text to match the current state.
...
* **Asynchronous Like Toggle** (Async Function): This function, toggleLike, enables users to like posts without reloading the page. It sends a POST request to /likePost using the Fetch API. Upon receiving a JSON response from the server, it instantly updates the DOM—modifying the like count, toggling the button's .liked class, and swapping the Font Awesome icon between 'regular' (outline) and 'solid' (filled)—to reflect the new state immediately.
...
The file serves as the interactivity layer, bridging the gap between static HTML views and dynamic user actions to create a seamless experience.

##### 4. createAdmin.js

This file defines the Administrative Seeding Script. It is a standalone utility designed to bypass standard registration restrictions and manually inject a privileged user account into the database.

The script implements four key steps required to safely create the admin user:

* **Database Connection** (Mongoose Setup): This section establishes a direct connection to the local MongoDB instance. Because this script runs independently of the main application (app.js), it must initiate its own database session to perform operations.
* **Password Security** (Bcrypt Implementation): This step manually hashes the hardcoded password ('1234') before storage. Since the User model does not include middleware to auto-hash passwords, this script must replicate the security logic found in the main signup controller to ensure the credential is not stored in plain text.
* **Privileged Account Creation** (Object Instantiation): This constructs the user document with specific hardcoded attributes. It assigns the username admin and explicitly grants the ['user', 'admin'] roles, conferring special permissions (such as global delete rights) that are inaccessible through the public signup form.
* **Connection Management** (Cleanup): This block handles the script's lifecycle using a finally clause. It guarantees that mongoose.connection.close() is called after the operation completes (success or fail), preventing the script from hanging indefinitely in the terminal.
...
The script executes the createAdmin function immediately upon running and logs the result to the console for verification.